  ...
  try {
    ...
    if (!user)
      return NextResponse.json(
        { message: "No user like this in database." },
        { status: 401 },
      );
    ...
    const allMessage: MessageDB[] = await MESSAGE.find({
      user: userIdInSession,
      createdAt: { $gte: sevenDaysAgo },
    });
    if (allMessage.length == 0) {
      console.log("no message found");
      return NextResponse.json(
        { message: "You havent have any messages in the last 7 days." },
        { status: 404 },
      );
    }
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayInWeek = dayWeek[
        date.getUTCDay()
      ] as roomMessageChartData["dayInWeek"];
    ...
    for (let i = 0; i < allMessage.length; i++) {
      ...
      // sometime it wont have
      if (!chartDataMap[dateStr]) {
        continue;
      }

      if (allMessage[i].user.toString() == userIdInSession) {
    ...
    console.error("Error in GET /api/charts/area-chart", error);
    return NextResponse.json({ message: "Server error!" }, { status: 500 });
  }
}
